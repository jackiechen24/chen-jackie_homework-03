...
Diagonal: density of curve describing distribution of variable (e.g. distribution of leaf dry weight right skewed); density functions somewhat similar to histogram

Bottom left diagonal: scatterplots relating all of these continuous variables to each other; bottom left plot - leaf dry weight on x-axis and total dry weight on y-axis

Calculated Pearson Correlation Coefficient; in the right: all the calculated correlation coefficients represented down below; Pearson Correlation Coefficient: total mass is significantly positively correlated with leaf dry weight

Choose 3 pairs of variables to compare. Make notes of those comparisons below.    

**Total mass and root mass are strongly positively correlated (Pearson's r = 0.858)**  
...
**Plants with larger biomass seem to have larger ranges. Grindelia has the highest biomass, while Acmispon has the lowest biomass.**
...
**Well watered plants tend to have larger biomass, which makes sense based on how plants work.**
...
**SLA doesn't really affect total mass if looking visually, but check correlation and compare evidence (visual and statistical).**

# Model construction

What is the null model?

0. **total mass is not predicted by any of the 3 predictors**

What is the saturated model?  

1. **total mass predicted by SLA, water treatment, and species**

saturated model: all the predictors are in there
...
Homoscedasticity: The standardized residuals might flare out a little big but potentially is not a big deal.

Normality: A little flare out at the tails, but that's normal.

Outliers: Nothing outside the dotted lines, nothing outside the 0.5 (not a big deal).

Diagnostics for the saturated model look good.
...
From the `MuMIn` (MuMIn stands for multimodel inference) package
...
Models ordered in increasing AIC, model with lowest AIC the best model that describes total mass without sacrificing complexity; the most complex model is the one that best describes total mass; delta: difference between some model and the best model, delta AIC for best model always going to be 0; models - increasing delta as you descend down the table

Out of the four models we have tried, the best model is the one that includes **all 3 predictors: SLA (specific leaf area), water treatment, and species**
...
- SLA slope: **Across drought treamtents and species with each 1 unit increase in SLA measured in mm^2/dry mass of a leaf, we expect an increase of total mass of -0.00025, which means that with the estimate and standard error, there is no effect of SLA on total mass.**  
Just because they are positively correlated, there is no predictive effect of SLA on total mass

- well watered estimate: **Across species and SLA, well watered species tend to be 0.12 ± (figure out how to write plus minus in text) 0.02 g heavier than drought stressed plants.**    
